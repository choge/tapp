...
class HMMResultSet(predictor.dataset.DataSet):
            ...
            for j in range(i + 1, len(self.models)):
                ...
                for name in self.identifiers:
                    if 'diff' not in self[name]:
                        self[name]['diff'] = {}
                    if keyname not in self[name]['diff'] and keyname_rev not in self[name]['diff']:
                        self[name]['diff'][keyname] = (self.get_likelihood(name, m1) - self.get_likelihood(name, m2)) / self.get_seqlen(name)
    ...
    def get_subset(self, what, how=None):
        ...
        if isinstance(what, str):  ## Assume it indicates an origin.
            ...
            if all([elem in self.identifiers for elem in what]):
                ...
                l = []
                for origin in what:
                    l.extend([how(name) for name in self.iter_names_by_origin(origin)])
                return l
